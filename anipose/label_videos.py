...
from .common import make_process_fun, natural_keys, get_nframes, true_basename, load_pose_2d
...
def visualize_labels_points(config, points, scores, bodyparts, vid_fname, outname):
    try:
...
def visualize_labels(config, labels_fname, vid_fname, outname):
    ...
    pts_scores, metadata = load_pose_2d(config['model_type'], labels_fname)

    bodyparts = metadata['bodyparts']

    points = pts_scores[:, :, 0, :2]
    scores = pts_scores[:, :, 0, 2]


    return visualize_labels_points(
        config, points, scores, bodyparts, vid_fname, outname)
...
def process_session(config, session_path, filtered=False):
    ...
    if config['model_type'] == 'deeplabcut':
        ...
        labels_fnames = glob(os.path.join(session_path, pipeline_pose, '*.predictions.slp'))
