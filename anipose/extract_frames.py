...
def load_2d_data(config):
    ...
    # for name in vid_names:
        ...
        points_proj = get_projected_points(bodyparts, pose_fname, cgroup, offsets_dict)
...
def extract_frames_bad(config, num_frames_pick=250):
    print('loading tracked data...')
    d = load_2d_data(config)

    print('getting outlier frames...')
    points = d['points']
    proj = d['proj']
    scores = d['scores']
    fnames = d['fnames']
    ...
    for ix in indexes:
        ...
        similar = (vidnum == vidnums) & (np.abs(framenums - framenum) <= 5)
        check[similar] = False
